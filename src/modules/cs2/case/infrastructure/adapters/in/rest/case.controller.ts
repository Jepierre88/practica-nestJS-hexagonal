...
import { CaseOpenResultDto, CaseOpenSkinDto } from './dtos/case-open-result.dto';
...
export class CaseController {
  ...
  @ApiOperation({ summary: 'Crear caja', description: 'Modo "price": envía price + skinIds (drop rates auto). Modo "rates": envía skins con dropRate (precio auto).' })
  ...
  async create(@Body() dto: CreateCaseDto): Promise<CaseResponseDto> {
    ...
    if (dto.skins && dto.skins.length > 0) {
      // Rates mode: skins with dropRates → auto-calculate price
      command = CreateCaseCommand.create({
        mode: 'rates',
        name: dto.name,
        skins: dto.skins,
      });
    } else {
      // Price mode: price + skinIds → auto-calculate drop rates
      command = CreateCaseCommand.create({
        mode: 'price',
        name: dto.name,
        price: dto.price ?? 0,
  ...
  @Post(':id/skins')
  @HttpCode(HttpStatus.CREATED)
  @ApiOperation({ summary: 'Agregar skin a caja', description: 'Los drop rates se recalculan automáticamente.' })
  @ApiParam({ name: 'id', type: String, description: 'ID de la caja' })
  @ApiResponse({ status: 201, type: CaseResponseDto })
  async addSkin(@Param() params: UuidParam, @Body() dto: AddSkinToCaseDto): Promise<CaseResponseDto> {
    const entity = await this.manageCaseSkinsUseCase.addSkin(
      AddSkinToCaseCommand.create({ caseId: params.id, skinId: dto.skinId }),
    );
    return this.dtoMapper.toResponse(entity);
  }
  ...
  async removeSkin(
    ...
    const entity = await this.manageCaseSkinsUseCase.removeSkin(
      RemoveSkinFromCaseCommand.create({ caseId, skinId }),
    );
    return this.dtoMapper.toResponse(entity);
  }
  ...
  @Post(':id/open')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Abrir caja', description: 'Selecciona un skin aleatorio basado en drop rates. Devuelve el resultado y todos los skins para la animación del spinner.' })
  @ApiParam({ name: 'id', type: String, description: 'ID de la caja' })
  ...
  async open(@Param() params: UuidParam): Promise<CaseOpenResultDto> {
    ...
    const mapSkin = (cs: { skin: { id?: string; name: string; price: number; skinFloat: number; weapon: { name: string }; collection: { name: string } }; dropRate: number }): CaseOpenSkinDto => ({
      skinId: cs.skin.id!,
